...
public class PageableTests {
    ...
    public void testContinuationToken() {
        PagedIterable<Pet> pagedIterable = tokenClient.requestHeaderResponseBody("token", "foo", "bar");
        Assertions.assertEquals(4, pagedIterable.stream().count());

        pagedIterable = tokenClient.requestHeaderResponseHeader("token", "foo", "bar");
        Assertions.assertEquals(4, pagedIterable.stream().count());

        pagedIterable = tokenClient.requestQueryResponseBody("token", "foo", "bar");
        Assertions.assertEquals(4, pagedIterable.stream().count());

        pagedIterable = tokenClient.requestQueryResponseHeader("token", "foo", "bar");
        ...
        Assertions.assertThrows(IllegalArgumentException.class,
            () -> tokenClient.requestQueryResponseHeader("token", "foo", "bar")
